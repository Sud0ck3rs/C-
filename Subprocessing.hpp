...
public:
    ...
    Process(const std::string& cmd, std::vector<std::string> argv, std::ostream& pipe_stdout)
        : in_filebuf(nullptr), out_filebuf(nullptr), err_filebuf(nullptr), in_stream(nullptr), out_stream(nullptr), err_stream(nullptr)
    {
    ...
    void set_cmd_Shell(const std::string &cmd, const std::vector<std::string> &args)
    {
        m_cmd = cmd;
        m_args = args;
    };
    ...
    void start()  // run process shell
    {
        run(m_cmd, m_args);
    };

    void start(const std::string &cmd, const std::vector<std::string> &args) //run process shell with args
    {
        run(cmd, args);
    };
